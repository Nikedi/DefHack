...
class MapPreferences:
    active_layers: Set[str] = field(
        default_factory=lambda: {"text", "voices", "sensors", "fused", "alerts"}
    )
...
class MapManager:
    ...
    def _prune(self, chat_id: int) -> None:
        ...
        if len(chat_obs) <= MAP_MAX_POINTS:
            return
        # remove stalest observations first
    ...
    async def render_snapshot(
        ...
        *,
        ...
        layers = set(layers) if layers is not None else set(prefs.active_layers)
        ...
        filtered = [
            obs
            for obs in chat_obs
            if obs.timestamp >= cutoff and self._matches_focus(obs, focus_terms) and self._matches_layers(obs, layers)
        ]
        ...
        rendered.applied_layers = layers
        rendered.focus_terms = focus_terms
        rendered.legend = self._build_legend(filtered, diff_info)
    ...
    async def set_layers(self, chat_id: int, layers: Sequence[str]) -> MapPreferences:
        valid = {"text", "voices", "sensors", "fused", "alerts"}
        chosen = {layer.lower() for layer in layers if layer.lower() in valid}
        if not chosen:
            chosen = set(valid)
        prefs = self._preferences.setdefault(chat_id, MapPreferences())
        prefs.active_layers = chosen
        return prefs
    ...
    def build_caption(self, result: MapRenderResult) -> str:
        ...
        if result.applied_layers:
            header_parts.append(f"Layers: {', '.join(sorted(result.applied_layers))}")
        if result.diff:
            diff = result.diff
            header_parts.append(
        ...
        if result.legend:
            header_parts.append(result.legend)
    ...
    def _build_legend(
        ...
        layers_present: Set[str] = {obs.source_type for obs in observations}
        legend_parts = [markers[layer] for layer in layers_present if layer in markers]
        if diff:
            if diff.new_ids:
                legend_parts.append("🟢 New")
            if diff.moved:
                legend_parts.append("🟣 Moved")
            if diff.stale:
    ...
    def _matches_layers(self, obs: MapObservation, layers: Set[str]) -> bool:
        if not layers:
            return True
        if obs.source_type in layers:
            return True
        if obs.priority <= 2 and "alerts" in layers:
            return True
        return False
