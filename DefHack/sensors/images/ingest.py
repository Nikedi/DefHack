...
from .camera import CameraWorker, ensure_folder
from .yolov8_person_pipeline import Yolov8PersonCaptionSchema
from ..SensorSchema import SensorObservationIn


_PACKAGE_ROOT = Path(__file__).resolve().parents[2]
_IMAGES_DIR = Path(__file__).resolve().parent

DEFAULT_API_URL = "http://172.20.10.5:8080/ingest/sensor"
DEFAULT_API_KEY = "583C55345736D7218355BCB51AA47"
DEFAULT_SAVE_FOLDER = _IMAGES_DIR / "current_image"
DEFAULT_BACKLOG_PATH = _PACKAGE_ROOT / "sensor_backlog.json"
...
def parse_args(argv: Sequence[str]) -> AppConfig:
    ...
    return AppConfig(
        ...
        backlog_file=args.backlog_file.expanduser().resolve(),
        save_folder=args.save_folder.expanduser().resolve(),
        source=args.source,
        test_images=[img.expanduser().resolve() for img in args.test_images] if args.test_images else None,
        ...
        device=args.device,
        caption_corpus=args.caption_corpus.expanduser().resolve() if args.caption_corpus else None,
...
def _canonicalise_payload(raw: dict[str, Any], *, unit_override: Optional[str] = None) -> dict[str, object]:
    ...
    structured: dict[str, object] = {
        "time": timestamp,
        "mgrs": _format_mgrs(
            mgrs_value if isinstance(mgrs_value, str) else (str(mgrs_value) if mgrs_value is not None else None)
        ),
        "what": what_value,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    camera = CameraWorker(
        src=config.source,
        test_images=[str(p) for p in config.test_images] if config.test_images else None,
    )
    empty_frame_runs = 0
