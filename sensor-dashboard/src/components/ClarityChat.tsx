...
export default function ClarityChat() {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="mil-panel mb-3 flex items-center justify-between">
        <div>
          <div className="font-bold">CLARITY • Sector D3</div>
          <div className="mil-muted text-xs">Secure • Encrypted • Last sync 00:00:30</div>
        </div>
        <div className="flex items-center gap-3">
          <div className="mil-muted text-sm">Connection: Encrypted</div>
          <div className="status-dot status-ok blink" title="Live" />
        </div>
      </div>

      <div className="mil-panel flex-1 overflow-auto p-4 scanline" style={{ minHeight: 420 }}>
              ...
              <div style={{ maxWidth: "78%" }}>
                <div className={`p-3 ${m.type === "assistant" ? "bg-black/30 border border-black/15" : "bg-black/20"} ${m.type === "assistant" ? "mil-panel" : "mil-panel"}`}>
                  <div className="flex items-baseline justify-between gap-3">
                    ...
                    <div className="mil-muted text-xs">{m.time}</div>
                  </div>
                  <pre className="whitespace-pre-wrap mt-2 text-sm" style={{ fontFamily: "inherit" }}>{m.text}</pre>
      ...
      <div className="mt-3 flex items-center gap-3">
        <input
          className="flex-1 p-2 bg-black/20 border border-black/15 mil-muted"
